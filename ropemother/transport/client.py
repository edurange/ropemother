#!/usr/bin/env python3
# ropemother/transport/client.py
...
class TransportClientError(MessageBusBaseException):
    """Base exception for transport client errors."""
    pass


class UnexpectedTransportFrameError(TypeError, TransportClientError):
    """Raised when a transport client receives an unexpected frame."""
    pass


class TransportRequestError(TransportClientError):
    """Raised when a transport request receives an error response."""
    error_code: str
...
class TransportClient(MessageEndpointFactory):
    ...
    def register_emitter(
        ...
        *,
        ...
        response = self._receive_expected_frame(RegisterEmitterResultFrame)
    ...
    def subscribe(
        ...
        *,
        ...
        response = self._receive_expected_frame(SubscribeResultFrame)
        self._registrations.apply_registrations(response.registrations)
    ...
    def _receive_delivery_frame(
        ...
        frames = self._receive_delivery_batch(
            subscription_id, min_count=1, max_count=1
        )
        return frames[0]

    def _receive_delivery_batch(
        self,
        ...
        *,
        ...
        while max_count is None or len(frames) < max_count:
            ...
            if frame is None and len(frames) < min_count:
                frame = self._receive_relevant_delivery_frame(subscription_id)
            elif frame is None:
    ...
    def _receive_relevant_delivery_frame(
        self, subscription_id: TransportSubscriptionID
    ) -> DeliveryFrame:
        while True:
            frame = self._channel.receive_frame()
...
class TransportEmitter(Emitter):
    ...
    def emit(
        ...
        *,
        ...
        self._client._receive_expected_frame(EmitResultFrame)
    ...
    def emit_request(
        ...
        *,
        ...
        result = self._client._receive_expected_frame(EmitResultFrame)
    ...
    def _emit_frame(
        ...
        *,
        ...
        msg_type_id = self._msg_type_id_for_emit(resolved_msg_type)
    ...
    def _msg_type_id_for_emit(self, msg_type: str) -> MessageTypeID:
        ...
        self._channel.send_frame(frame)
        response = self._client._receive_expected_frame(
            RegisterMessageTypeResultFrame
        )
        self._registrations.apply_registrations(response.registrations)
...
class TransportReceiver(Receiver):
    """Synchronous receiver backed by transport frames."""
    _client: TransportClient
    ...
    def _receive_batch(
        ...
        frames = self._client._receive_delivery_batch(
